function TriMeshHandle = drawTriangulatedCartoMesh(Mesh,ResHandle,Apex,Depths)
%CARTO = CARTO_Pre2;
smoothingKernel = [1 1];
if ~exist('Depths','var')
    Depths = 0;
...
for imageIdx = 1:length(ResHandle)
    xdata = get(ResHandle(imageIdx),'xdata');
    ydata = get(ResHandle(imageIdx),'ydata');
    zdata = get(ResHandle(imageIdx),'zdata');
    cdata = get(ResHandle(imageIdx),'cdata');
    if length(get(ResHandle(imageIdx),'alphadata')) == 1
        adata = ~isnan(cdata);
    else
        adata = padarray(get(ResHandle(imageIdx),'alphadata'),[1 1],'post');
    end
    ...
    for MeshIdx = 1:length(Mesh)
    mx = [mx;Mesh(MeshIdx).Vertices.X];
    my = [my;Mesh(MeshIdx).Vertices.Y];
    mz = [mz;Mesh(MeshIdx).Vertices.Z];
    end
