function CARTO = displayCartoStudy(CARTO,figureHandle)

if ~exist('figureHandle','var');
    CARTO.figureHandle = 200;
else
    CARTO.figureHandle = figureHandle;
end
...
if isfield(CARTO,'Mesh')
    for MeshIndex = 1:length(CARTO.Mesh)
        CARTO.MeshHandle(MeshIndex) = drawCartoMesh(CARTO.Mesh(MeshIndex),'LAT',[CARTO.ActivationTimeRange{1}(1) CARTO.ActivationTimeRange{2}(2)],ActivationColorMap,'facelight','gouraud','EdgeColor','none');
    end
end
...
legend(CARTO.PointsHandle,'location','northeast')
end
colormap jet
axis image
axis xy
