function TriMeshHandle = drawTriangulatedCartoMesh_Cartoon(CARTO,Depths)
%CARTO = CARTO_Pre2;
smoothingKernel = [1 1];
if ~exist('Depths','var')
    Depths = 0;
...
for i = 1%:length(CARTO.CartoPoints)
    d(:,i) = sqrt(...
        (x-CARTO.CartoPoints(i).Position3D(1)).^2 +...
        (y-CARTO.CartoPoints(i).Position3D(2)).^2 +...
        (z-CARTO.CartoPoints(i).Position3D(3)).^2);
end
    
    dmin = min(d,[],2);
    %c = 1+4.*(dmin<5);
    c = max(1,min(5,30*exp(-0.5.*dmin)));
    x = x(a>0);
    y = y(a>0);
    z = z(a>0);
    c = c(a>0);
